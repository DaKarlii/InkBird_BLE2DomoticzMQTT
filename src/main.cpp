...
char user_mqtt_topic[255] = DOMINTOPIC;
    ...
    bool isNotify)
{
  ...
  for (int i = 0; i < length; i += 2)
  {
    ...
    if(((int)temp)<PROBEERRORVALUE)
    {
      if (mqttIsConnected())
      {
        //char user_mqtt_topic[255];
        char domtemppayload[255];
        //sprintf(user_mqtt_topic, DOMINTOPIC);
        sprintf(domtemppayload, "{\"command\":\"udevice\",\"idx\":%d,\"nvalue\":0,\"svalue\":\"%d\"}", probeIDX, (int)temp );
        mqttclient.publish(user_mqtt_topic, domtemppayload);
        ESP_LOGI("BBQ", "Publish %d to topic %s with payload %s", (int)temp, user_mqtt_topic, domtemppayload);
      }
    }
    probeId++;
    probeIDX++;
    ...
    bool isNotify)
{
  ...
  case 0x24:
  {
    ...
    if (mqttIsConnected())
    {
      char dombattpayload[255];
      sprintf(dombattpayload, "{\"command\":\"udevice\",\"idx\":%d,\"nvalue\":0,\"svalue\":\"%d\"}", DOMBATTIDX, (int)battery_percent);
      mqttclient.publish(user_mqtt_topic, dombattpayload);
      ESP_LOGI("BBQ", "Publish %f to topic %s with payload %s", battery_percent, user_mqtt_topic, dombattpayload);
    }
